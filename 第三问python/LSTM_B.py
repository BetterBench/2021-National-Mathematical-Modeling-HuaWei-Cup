...
def f_data(train_1, train_2):
    ...
    if train_1 == True:
        ...
        D1HP = pd.read_excel('/media/mgege007/Compitition/Math/data/data_2_knn_AQI.xlsx',
                             sheet_name='监测点B逐小时污染物浓度与气象一次预报数据', index_col="time", parse_dates=True)
        ...
        checkpoint = tf.keras.callbacks.ModelCheckpoint('./weight/B_weight_f.tf', save_format='tf',
                                                        verbose=0, save_best_only=True, save_weights_only=True)
        k = trainX.shape[0] % BATCH_START
        trainX, trainY = trainX[k:], trainY[k:]
        model.load_weights('./weight/B_weight_f.tf')
        ...
        df_result.to_csv("3_BHP.csv", index=False)
        print()
    if train_2 == True:
        # 开始第二轮训练
        # load_dataset
        D1HA = pd.read_excel('/media/mgege007/Compitition/Math/data/data_2_knn_AQI.xlsx',
                             sheet_name='监测点B逐小时污染物浓度与气象实测数据', index_col="time", parse_dates=True)
        ...
        start = time.time()
        model.compile(metrics=['accuracy'], loss='mse', optimizer='adam')
        checkpoint = tf.keras.callbacks.ModelCheckpoint('./weight/B_weight_s.tf', save_format='tf',
                                                        verbose=0, save_best_only=True, save_weights_only=True)
        k = trainX.shape[0] % BATCH_START
        trainX, trainY = trainX[k:], trainY[k:]
        model.load_weights('./weight/B_weight_s.tf')
        # history = model.fit(trainX, trainY, batch_size=BATCH_START, epochs=EPOCH,
        #                     validation_split=0.1,
        #                     callbacks=[checkpoint],
        #                     verbose=1)
        ...
        result = scaler.inverse_transform(result[0, :, :])
        df_result = pd.DataFrame(result)
        df_result.to_csv("3_BHA.csv", index=False)
        print()
...
def pre_data():
    ...
    D1DA = pd.read_excel('/media/mgege007/Compitition/Math/data/data_2_knn_AQI.xlsx',
                         sheet_name='监测点B逐日污染物浓度实测数据', index_col="time", parse_dates=True)
    ...
    start = time.time()
    model.compile(metrics=['accuracy'], loss='mse', optimizer='adam')
    checkpoint = tf.keras.callbacks.ModelCheckpoint('./weight/B_weight_t.tf', save_format='tf',
                                                    verbose=0, save_best_only=True, save_weights_only=True)
    k = trainX.shape[0] % BATCH_START
    trainX, trainY = trainX[k:], trainY[k:]
    model.load_weights('./weight/B_weight_t.tf')
    # history = model.fit(trainX, trainY, batch_size=BATCH_START, epochs=EPOCH,
    #                     validation_split=0.1,
    #                     callbacks=[checkpoint],
    #                     verbose=1)
    ...
    result = scaler.inverse_transform(result[0,:,:])
    df_result = pd.DataFrame(result)
    df_result.to_csv("3_BDA.csv",index=False)
    print()
